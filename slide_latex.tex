\documentclass[dvipdfmx,ja=standard]{beamer} % 1行目をupLaTeX用に変更
% \usepackage{luatexja} % upLaTeXでは不要なので削除（またはコメントアウト）
% \renewcommand{\kanjifamilydefault}{\gtdefault} % 1行目の設定で自動でゴシックになるので不要
...
% 最初のタイトルスライド（\maketitle は通常独立したframeにします）
\begin{frame}
    \maketitle
\end{frame}

\begin{frame}
    % タイポ修正：alarted -> alerted
